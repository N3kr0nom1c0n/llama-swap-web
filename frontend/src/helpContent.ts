    ...
    articles: [
      {
        ...
        details: [
          "The app is designed to close the gap between 'I downloaded a GGUF' and 'llama-swap can actually serve it'. A completed download only proves the bytes landed under the selected target rig's model root. A managed model entry is what gives the file a model name, role, command flags, GPU placement, matrix behavior, aliases, and TTL.",
          "The config step is intentionally staged. Config Preview renders YAML for the selected target rig, validates known constraints on that rig, and shows a diff. Apply creates a backup first and writes that rig's config file only after you approve the staged preview.",
          "The restart step is separate from apply. SSH rigs restart with their configured remote command. Local single-host installs can use the optional Docker socket restart control. If restart controls are disabled or unhealthy, restart llama-swap outside the manager.",
        ],
        workflow: [
          "Select the correct target rig in the top bar or Settings.",
          "Choose a source in Import Model: Hugging Face URL, local upload, or scan existing model files on that rig.",
          "Select the primary GGUF and companion files, then download or stage them under the correct role directory on the target rig.",
        ...
        examples: [
          "SSH target file path on rig: /home/n3kr0/Repos/llama.cpp/models/chat/model-name/model.Q4_K_M.gguf",
        ...
        relatedTerms: ["target rig", "GGUF", "managed model", "config preview", "matrix", "restart control"],
        relatedArticles: ["article-target-rigs", "article-import-model", "article-models-page", "article-config-preview"],
        tags: ["pipeline", "download", "installed", "apply", "restart"],
      },
      {
        id: "article-target-rigs",
        title: "Target Rigs",
        category: "Architecture",
        summary:
          "A target rig is the machine that owns llama-swap operations: downloads, scans, GPU detection, config writes, backups, and restarts.",
        details: [
          "The manager UI can run on one host while llama-swap runs on another. In that setup, the manager must not write model files, config, or backups to its own filesystem. It connects to each target rig over SSH and performs those operations there.",
          "The selected target rig is shown in the top bar. Import Model, Models, GPU Planner, Config Preview, health checks, downloads, uploads, scans, backups, and restart controls are scoped to that selection.",
          "Use local mode only when the manager and llama-swap share bind-mounted storage on the same host. Use SSH mode for a separate AI rig, such as manager on 192.168.42.10 and llama-swap on 192.168.42.40.",
        ],
        settings: [
          {
            name: "Mode",
            purpose: "Chooses local bind-mounted behavior or SSH remote behavior.",
            ifYouChange: "All rig-scoped operations switch between local filesystem calls and SSH commands.",
            tradeoff: "Wrong mode makes paths look valid while writes happen on the wrong machine.",
            where: "Settings -> Target Rigs",
          },
          {
            name: "SSH host and username",
            purpose: "The remote machine and account used for model, config, GPU, and restart operations.",
            ifYouChange: "The manager targets a different rig.",
            tradeoff: "The SSH user needs enough permission to write model/config/backup paths and run the restart command.",
            where: "Settings -> Target Rigs",
          },
          {
            name: "SSH key path",
            purpose: "Path inside the manager container to the private key used for the target rig.",
            ifYouChange: "Connection attempts use a different key.",
            tradeoff: "The key must be mounted read-only into the manager and protected like a credential.",
            where: "Settings -> Target Rigs",
          },
        ],
        examples: [
          "Manager host: 192.168.42.10 running the web app.",
          "Target rig: 192.168.42.40 running llama-swap and owning the model disk.",
          "SSH key path in manager container: /data/ssh/id_ed25519.",
        ],
        warnings: [
          "If 192.168.42.40 is powered off, SSH target-rig operations should fail instead of silently writing to 192.168.42.10.",
          "A destination like /models/chat/name in generated commands is a llama-swap container path. The SSH write path may be a different real path on the target rig.",
        ],
        relatedTerms: ["target rig", "SSH", "SSH key", "target model root", "llama-swap model root"],
        relatedArticles: ["article-container-paths", "article-settings-page", "article-restart-control"],
        tags: ["target rig", "ssh", "remote", "multi rig", "settings"],
      },
      {
        id: "article-container-paths",
        title: "Target Paths Versus Container Paths",
        category: "Architecture",
        summary:
          "SSH writes use the target rig's real filesystem path, while generated llama-swap config uses the path visible inside the llama-swap container.",
        details: [
          "For an SSH target rig, Target model root is where the manager writes files over SSH. Llama-swap model root is what the llama-swap container sees. The manager maps between those two paths so bytes land on the remote disk while commands still use /models.",
          "For a local single-host install, Docker bind mounts can make the same storage visible to both containers at /models. In that case target model root and llama-swap model root are often both /models.",
          "Config apply writes the selected rig's config path. On SSH rigs that is a remote file written over SSH with a backup first. On local rigs it is the mounted config file inside the manager container.",
        ],
        settings: [
          {
            name: "Target model root",
            purpose: "Real model directory on the selected target rig where downloads, uploads, and scans happen.",
            ifYouChange: "Future writes and scans move to a different path on that rig.",
            tradeoff: "If it does not point at the disk mounted into llama-swap, downloads succeed but llama-swap cannot load them.",
            where: "Settings -> Target Rigs",
          },
          {
          ...
          {
            name: "Config path",
            purpose: "The selected rig's config.yaml file that the manager previews, backs up, writes, and restores.",
        ...
        examples: [
          "Target model root on rig: /home/n3kr0/Repos/llama.cpp/models",
          "Llama-swap model root in config: /models",
          "Generated command path: /models/chat/Celeste-12B/model.gguf",
        ],
        relatedTerms: ["target model root", "llama-swap model root", "container files", "bind mount", "backup", "config preview"],
        relatedArticles: ["article-target-rigs", "article-path-settings", "article-config-preview"],
    ...
    articles: [
      ...
      },
      {
        ...
        details: [
          ...
          "Resolved files start unchecked so you intentionally select only the primary GGUF and companion files you want. Preview Destination explains where those selected files will land on the selected target rig before the download starts.",
          ...
          "Scan Existing Files reads the selected target rig's model root. On SSH rigs, the scan happens remotely instead of looking at the manager host.",
        ],
        workflow: [
          "Select the correct target rig.",
          "Resolve a Hugging Face URL or scan that rig's model root.",
          "Select the primary GGUF and required companions.",
          "Start the download and watch the progress row until it reaches completed on the target rig.",
      ...
      },
      {
        ...
        details: [
          "CUDA indexes are what llama.cpp and CUDA_VISIBLE_DEVICES use. Keep them aligned with nvidia-smi on the selected target rig. If the physical cards change, update the planner before tuning model entries.",
      ...
      },
      {
        ...
        details: [
          "Preview generates the config from current settings and saved model entries for the selected target rig. It should show both the YAML and the unified diff against that rig's active config file.",
          ...
          "The llama-swap Runtime panel is target-rig aware. SSH rigs report the configured remote restart command. Local rigs can use Docker socket restart controls when explicitly enabled.",
        ],
        warnings: [
          ...
          "Docker socket access is host-level control for the manager host only. Use SSH restart commands for remote llama-swap rigs.",
        ],
        relatedTerms: ["target rig", "config preview", "backup", "restart control", "Docker socket", "validation"],
        relatedArticles: ["article-target-rigs", "article-restart-control", "article-config-rollback"],
      ...
      },
      {
        ...
        category: "Page",
        summary:
          "Settings controls target rigs, app-wide assumptions, token handling, role directories, command defaults, disk safety, and optional restart behavior.",
        details: [
          "Target Rigs decide which machine owns downloads, scans, GPU detection, config apply, backups, and restart. In split-host installs, this is the most important Settings section.",
          ...
          "Restart settings are separate from model settings because they can grant either SSH command execution on the target rig or Docker socket access on a local host.",
        ],
        relatedTerms: ["target rig", "HF token", "model root", "role directory", "restart control", "disk safety"],
        relatedArticles: ["article-target-rigs", "article-path-settings", "article-default-presets", "article-restart-control"],
    ...
    articles: [
      {
        ...
        details: [
          "For SSH rigs, per-rig target paths are configured in Target Rigs. Those decide the remote model root, remote config path, remote backups dir, remote temp dir, and llama-swap container model root.",
          "Global app paths are still used for manager state and local-mode defaults. Use /data for SQLite state. Use /data/tmp for manager-side temporary upload staging when streaming files to SSH rigs.",
          "In local single-host mode, keep manager paths inside the container. The Compose file maps those container paths to host storage so generated config stays portable across the manager and llama-swap containers.",
        ],
        settings: [
          {
            name: "Target model root",
            purpose: "Remote model directory on an SSH target rig, or local model directory for a local rig.",
            ifYouChange: "Future downloads, uploads, scans, and file checks use a different root.",
            tradeoff: "Wrong values put files on a disk llama-swap cannot see.",
            where: "Settings -> Target Rigs",
          },
          {
            name: "Backups dir",
            ...
            where: "Settings -> Target Rigs for SSH, Settings -> Paths for local defaults",
          },
          {
            name: "Data dir",
        ...
        relatedTerms: ["target rig", "bind mount", "backup", "SQLite", "model root"],
        relatedArticles: ["article-target-rigs", "article-container-paths"],
      ...
      },
      {
        ...
        category: "Operations",
        summary:
          "Restart control is an optional manual button that restarts llama-swap on the selected target rig after a config apply or restore.",
        details: [
          "Enabling restart control does not restart anything by itself. It allows Config Preview to show runtime availability and expose a restart button.",
          "For SSH rigs, the restart button runs the rig's configured restart command over SSH. Use a narrow command such as docker restart llama-swap or a compose restart command on that remote host.",
          "For local single-host rigs, Docker socket restart requires /var/run/docker.sock mounted and readable. Because the app runs as a non-root user, Compose may also need group_add set to the Docker socket group id.",
        ...
        settings: [
          {
            name: "Target rig restart command",
            purpose: "Command run over SSH for a remote target rig.",
            ifYouChange: "Config Preview restarts llama-swap using the new remote command.",
            tradeoff: "The SSH user can run exactly what you configure here; keep it scoped to llama-swap.",
            where: "Settings -> Target Rigs",
          },
          {
            name: "Enable llama-swap restart control",
            purpose: "Turns on local Docker socket status and restart actions for local rigs.",
            ifYouChange: "Config Preview can show local runtime availability and a restart button.",
            tradeoff: "Docker socket access can control the manager host. Keep the app trusted-LAN only.",
        ...
        relatedTerms: ["target rig", "SSH", "restart control", "Docker socket", "container name", "backup"],
        relatedArticles: ["article-target-rigs", "article-config-preview"],
    ...
    articles: [
      {
        ...
        details: [
          "Open Import Model with the same target rig selected and look for Ready Downloads. If the downloaded job appears there, use Create Managed Model. If it does not, scan existing files on that rig and create the model from the GGUF on disk.",
          ...
          "Apply config with backup on the selected target rig. Restart llama-swap manually or with the runtime panel. The model will not be served by llama-swap until llama-swap loads the updated config.",
        ],
        workflow: [
      ...
      },
      {
        ...
        details: [
          ...
          "If restore fails because config is not writable, inspect the selected target rig's config path. SSH rigs need remote file permissions; local rigs need the bind-mounted config file writable inside the manager.",
        ],
    ...
    articles: [
      {
        ...
        details: [
          "Start in Settings and create or select the target rig that actually runs llama-swap. For a split-host install, that means the SSH rig, not the manager host.",
          "Confirm target health, config writability, target model root, llama-swap model root, backup path, temp path, and restart command before importing anything; otherwise you can download files but not apply or restart cleanly.",
          "Save a read-only HF token if the repo is private or gated. Then use GPU Planner to confirm CUDA indexes and VRAM on the selected target rig.",
        ...
        workflow: [
          "Settings -> create/select target rig and check health.",
          "Settings -> confirm target model root, llama-swap model root, config path, backups dir, temp dir, HF token status, and restart command.",
          "GPU Planner -> confirm CUDA indexes and VRAM match the selected rig.",
      ...
      },
      {
        ...
        details: [
          "Use Dashboard as the status board for the selected target rig, not as the place to configure models. It should show whether there is unfinished work: failed jobs, completed downloads that need a model entry, or config problems.",
    ...
    articles: [
      {
        ...
        details: [
          ...
          "Resolve Files talks to Hugging Face and classifies files. Files are unchecked by default so you choose the exact GGUF, shard set, mmproj, template, and tokenizer files. Scan Existing Files searches the selected target rig. Upload GGUF streams the browser file to the selected target rig without using Hugging Face.",
          "Preview Destination creates the staged import plan for the selected target rig. Start Download creates a rig-side download job. The Download Queue is the durable job control surface with progress, destination, cancel, retry, create, and Clear Finished actions.",
        ],
        settings: [
          {
            name: "Role",
          ...
          },
          {
            ...
            ifYouChange: "You bypass Hugging Face and write the uploaded file into the selected target rig's model root.",
          ...
          },
          {
            ...
            ifYouChange: "Cancel stops queued/running work, Retry reruns failed/cancelled jobs, Create turns completed jobs into managed models, and Clear Finished hides terminal jobs for the selected rig.",
      ...
      },
      {
        ...
        details: [
          ...
          "Target Rig fields decide where model files, config, backups, remote temp files, GPU detection, and restart commands happen. Global path fields decide where manager state and local-mode defaults live.",
        ...
        settings: [
          {
            name: "Target Rig",
            purpose: "Selected machine that owns llama-swap operations.",
            ifYouChange: "Imports, scans, GPU planner data, config preview/apply, backups, and restart actions switch to that machine.",
            tradeoff: "Selecting the wrong rig can make the UI look empty or write to the wrong place.",
            where: "Top bar and Settings -> Target Rigs",
          },
          {
            name: "SSH host / username / key",
            purpose: "Connection details for remote target rigs.",
            ifYouChange: "Remote operations use a different SSH destination or credential.",
            tradeoff: "Bad SSH settings make downloads, scans, GPU detection, config apply, and restart unavailable.",
            where: "Settings -> Target Rigs",
          },
          {
            name: "Target config path",
            purpose: "Config file on the selected rig that apply and restore write.",
            ifYouChange: "Config Preview targets a different file.",
            tradeoff: "Wrong path can update a file llama-swap is not actually using.",
            where: "Settings -> Target Rigs",
          },
          {
      ...
      },
      {
        ...
        details: [
          ...
          "Generated YAML and Diff are copyable code blocks. The llama-swap Runtime panel shows selected target rig restart availability. Backups lists timestamped configs from the selected rig and Restore replaces that rig's active file with a selected backup.",
        ],
        settings: [
          {
            name: "Preview Config",
          ...
          {
            name: "Apply Config",
            purpose: "Validate, backup, and write the selected target rig's config file.",
      ...
      },
      {
        ...
        details: [
          "Detect GPUs reads the selected target rig. SSH rigs run nvidia-smi remotely; local rigs run it in the manager runtime when available. Use Detected GPUs to replace the draft while preserving role and note fields by CUDA index.",
    ...
    articles: [
      {
        ...
        details: [
          "A production-ready SSH install has persistent manager /data, a mounted read-only SSH key, a saved target rig, writable remote model/config/backup/temp paths, and a restart command that only affects llama-swap. The app should survive container recreation without losing settings, jobs, or model entries.",
          "The LAN trust boundary matters because this app intentionally has no login. SSH keys, HF tokens, remote restart commands, and optional local Docker socket access are powerful even on a trusted network.",
          "Smoke test the exact workflow on the real target rig: health endpoint OK, settings persist, target health OK, HF resolve works for public repos, scan existing files works on the rig, config preview validates, apply creates a remote backup, restore creates a safety backup, and restart status reports the real llama-swap service.",
        ],
        workflow: [
          "Confirm /data/manager.db exists and is writable.",
          "Confirm SSH key mount is read-only and target rig health passes.",
          "Confirm target backups dir is durable and retention is intentional.",
          "Confirm generated commands use /models and never host-only paths.",
          "Confirm HF token status never exposes the token value.",
          "Confirm Docker socket is mounted only for local single-host restart controls, not for remote SSH rigs.",
        ...
        warnings: [
          ...
          "An SSH key that can write models, write config, and restart llama-swap is a production credential.",
          "Docker socket exposure is powerful enough to affect the manager host, not just this app.",
        ],
        relatedTerms: ["target rig", "SSH key", "Docker socket exposure", "SQLite", "backup retention", "HF token", "CORS"],
        relatedArticles: ["article-target-rigs", "article-security-boundaries", "article-backup-policy", "article-deploy-upgrade"],
      ...
      },
      {
        ...
        details: [
          ...
          "File writes must stay inside allowed target roots: target model root, target backups dir, target temp dir, /data, and local upload staging. Upload and scan paths should never allow traversal outside those roots.",
          "SSH keys should be mounted read-only into the manager and scoped to the rig account that owns model/config operations. The restart command should be narrow and predictable.",
          "CORS should stay closed unless a specific trusted frontend origin needs API access. Do not expose this app through a public reverse proxy without adding authentication and re-evaluating SSH key, token, and Docker socket exposure.",
        ],
        settings: [
          {
            name: "SSH key exposure",
            purpose: "Allows the manager to write models/config and run checks on a remote target rig.",
            ifYouChange: "Remote operations become available or unavailable for SSH rigs.",
            tradeoff: "The key is a credential. Mount it read-only and keep the SSH account scoped.",
            where: "Compose mount plus Settings -> Target Rigs",
          },
          {
            name: "Docker socket exposure",
            purpose: "Allows local restart controls to inspect and restart the configured llama-swap container.",
            ifYouChange: "Mounting or enabling it turns restart from unavailable into actionable.",
            tradeoff: "The socket can control Docker on the manager host; LAN-only does not make that harmless.",
        ...
        relatedTerms: ["SSH key", "Docker socket exposure", "HF token", "path traversal", "CORS", "Upload GGUF"],
        relatedArticles: ["article-target-rigs", "article-production-readiness", "article-restart-control"],
      ...
      },
      {
        ...
        details: [
          "The app image should be replaceable. Manager runtime state belongs in mounts: /data for SQLite, the manager env file for HF_TOKEN if used, and a read-only SSH key directory for remote rigs. Remote model files, backups, and config live on the selected target rig.",
          "Before upgrades, verify git state, rebuild or pull the image, recreate only the manager container, then smoke test /api/health and the Help, Settings, Import Model, Models, GPU Planner, and Config Preview pages against the real target rig.",
          "Do not overwrite the remote .env, target model root, target backups dir, or live llama-swap config unintentionally during code syncs. Code deploy and runtime state are different things.",
        ],
        workflow: [
          "Backup current config or confirm recent backup exists.",
          "Sync code while excluding .env, /data, SSH keys, remote backups, remote models, live configs, and logs.",
        ...
        relatedTerms: ["target rig", "SSH key", "SQLite", "HF token", "backup", "health endpoint"],
        relatedArticles: ["article-production-readiness", "article-target-rigs", "article-container-paths"],
...
export const impactRows: ImpactRow[] = [
  ...
  },
  {
    ...
    cost: "Requires either an SSH restart command on the target rig or local Docker socket access.",
    where: "Settings -> Target Rigs or Settings -> Restart control",
    articleId: "article-restart-control",
  },
  {
    id: "impact-target-rig",
    setting: "target rig",
    change: "Switch selected rig",
    canNet: "The same UI can manage multiple llama-swap machines without moving files by hand.",
    cost: "All rig-scoped views change at once, so verify the selected rig before downloading, applying config, or restarting.",
    where: "Top bar and Settings -> Target Rigs",
    articleId: "article-target-rigs",
  },
  {
    id: "impact-hf-token",
    setting: "HF token",
...
export const glossaryTerms: GlossaryTerm[] = [
  ...
  },
  {
    term: "target rig",
    slug: "target-rig",
    shortDefinition: "Machine that owns llama-swap operations for the selected view.",
    details: "Downloads, uploads, scans, GPU detection, config apply, backups, and restart controls run against the selected target rig.",
    articleId: "article-target-rigs",
    relatedTerms: ["SSH", "target model root", "llama-swap model root"],
  },
  {
    term: "SSH",
    slug: "ssh",
    shortDefinition: "Remote command and file transport used for split-host target rigs.",
    details: "SSH lets the manager stay self-contained while performing model, config, GPU, and restart work on the llama-swap rig.",
    articleId: "article-target-rigs",
    relatedTerms: ["SSH key", "target rig"],
  },
  {
    term: "SSH key",
    slug: "ssh-key",
    shortDefinition: "Credential mounted into the manager so it can reach a remote target rig.",
    details: "Mount it read-only and use an account scoped to model/config operations and the llama-swap restart command.",
    articleId: "article-security-boundaries",
    relatedTerms: ["SSH", "target rig"],
  },
  {
    term: "revision",
    slug: "revision",
  ...
  },
  {
    ...
    shortDefinition: "Optional manual restart button for llama-swap on the selected target rig.",
    details: "SSH rigs use the configured remote restart command. Local rigs can use Docker socket access and a matching container name.",
    articleId: "article-restart-control",
  },
  {
    term: "Docker socket",
  ...
  },
  {
    ...
    details: "For SSH rigs, distinguish target model root on the rig from llama-swap model root inside the llama-swap container.",
    articleId: "article-container-paths",
  },
  {
    term: "target model root",
    slug: "target-model-root",
    shortDefinition: "Real filesystem directory on the selected rig where model files are written.",
    details: "In SSH mode this is the remote path that receives Hugging Face downloads and uploads.",
    articleId: "article-container-paths",
    relatedTerms: ["llama-swap model root", "target rig"],
  },
  {
    term: "llama-swap model root",
    slug: "llama-swap-model-root",
    shortDefinition: "Model directory path visible inside the llama-swap container.",
    details: "Generated commands use this path, usually /models, even when SSH writes to a different host path on the rig.",
    articleId: "article-container-paths",
    relatedTerms: ["target model root", "container files"],
  },
  {
    term: "role directory",
    slug: "role-directory",
  ...
  },
  {
    ...
    details: "The manager should constrain writes to approved target roots such as model, backup, data, and temp directories.",
    articleId: "article-security-boundaries",
  },
  {
    term: "${PORT}",
