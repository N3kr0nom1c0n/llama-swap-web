...
import { ChevronRight, Cpu, FileSearch, FileText, GitBranch, HardDrive, Plus, Save, SlidersHorizontal, Terminal } from "lucide-react";
    ...
    <div className="page">
      ...
      <section className="models-layout">
        ...
        <div className="panel model-editor">
          <SectionGroup title="Identity" icon={<FileText size={15} aria-hidden="true" />} defaultOpen summary={`${draft.id || "new model"} · ${draft.role}`}>
          ...
          <SectionGroup title="Files" icon={<HardDrive size={15} aria-hidden="true" />} summary={`${draft.container_files.length} file(s) set`}>
          ...
          <SectionGroup title="GPU Placement" icon={<Cpu size={15} aria-hidden="true" />} summary={`GPU ${draft.gpu_devices.join(", ") || "none"}`}>
            <div className="form-grid">
          ...
          <SectionGroup title="llama-server Flags" icon={<SlidersHorizontal size={15} aria-hidden="true" />} summary={`ctx-size ${draft.llama_flags?.ctx_size ?? "default"}`}>
            <FlagEditor model={draft} onChange={update} />
          </SectionGroup>

          <SectionGroup title="Matrix & Behavior" icon={<GitBranch size={15} aria-hidden="true" />} summary={`${draft.matrix_behavior} · evict cost ${draft.evict_cost ?? "default"}`}>
            <div className="form-grid">
          ...
          <SectionGroup title="Advanced" icon={<Terminal size={15} aria-hidden="true" />} summary={draft.raw_cmd_override ? "override set" : "no override"}>
          ...
          <div className="sticky-save">
            <span className="sticky-save-hint">{save.isSuccess ? "Saved." : save.error ? save.error.message : "Unsaved changes"}</span>
...
function SectionGroup({
  ...
  return (
    <div className="section-group">
      <button type="button" className={`section-toggle${open ? " open" : ""}`} onClick={() => setOpen((value) => !value)}>
        <div className="section-toggle-left">
          <span className="section-toggle-icon">{icon}</span>
          <h3>{title}</h3>
          {!open && summary ? <span className="section-toggle-summary">{summary}</span> : null}
        </div>
        <ChevronRight size={14} className="section-toggle-chevron" aria-hidden="true" />
      </button>
